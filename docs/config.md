...
| `decisions` | no       | `{ dir, comment }` — `dir` is the append-only policy decision log (default `~/.beflow/decisions`); `comment` (default `true`) posts a receipt of each decision on the tracker issue, set `false` to opt out. The receipt body is the overridable [`decision-receipt.md`](prompts.md#decision-receipt-template-decision-receiptmd) template. |
